...
- **Active-call lock on Person** — `Person.activeCallStatus` (`IDLE` / `CALLING` / `COOLDOWN`) and `Person.activeCallCooldownUntil` are kept in sync by the PBX webhooks so concurrent dialers (human operator + AI agent + n8n + extra staff) can avoid colliding on the same client. The App publishes; consumers read. See [`docs/ACTIVE_CALL_LOCK.md`](docs/ACTIVE_CALL_LOCK.md).
