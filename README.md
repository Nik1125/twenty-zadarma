...
| `ZADARMA_SMS_SENDER_ID` | Optional. Pre-approved alphanumeric Sender ID from your Zadarma cabinet (SMS → Sender ID — the same list as the "Od kogo" dropdown when sending manually), e.g. `Hyalual`. When set, outbound SMS show this name instead of the raw DID. **One-way only** — recipients cannot reply to an alphanumeric sender (same as "TeamSale"); use only for notifications where no reply is expected. Leave blank (default) to send from the phone number (repliable); clearing it is the instant rollback if a carrier rejects the Sender ID. |
